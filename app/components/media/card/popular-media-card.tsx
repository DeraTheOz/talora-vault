...
type MediaItem = {
  ...
  | { title: string; release_date: string; name?: never; first_air_date?: never }
  | { name: string; first_air_date: string; title?: never; release_date?: never }
);
...
export default function MediaCard({ item, mediaType }: MediaCardProps) {
  ...
  const mediaLabel = isMovie ? t("movie") : t("tvSeries");
  const mediaIcon = isMovie ? Film02Icon : Tv01Icon;
  const href = isMovie ? `/movies/${item.id}` : `/series/${item.id}`;

  const title = isMovie ? ("title" in item ? (item as { title: string }).title : "") : ("name" in item ? (item as { name: string }).name : "");
  const releaseDate = isMovie ? ("release_date" in item ? (item as { release_date: string }).release_date : null) : ("first_air_date" in item ? (item as { first_air_date: string }).first_air_date : null);
